...
def check_name(platform_name=None, report=None):
    ...
    for ts in report["testsuites"]:
        if ts["platform"].replace("/", "_") != platform_name:
            err_cnt += 1
...
def check_version_consistent(report=None, version_expected=None):
    """
    ...
    """
    version_report = report['environment']['zephyr_version']
    if version_report == version_expected:
        return True

    print(f"In report: {version_report} vs expected {version_expected}")
    return False
